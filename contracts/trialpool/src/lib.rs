...
impl Contract {
    #[init]
    pub fn new(owner_id: AccountId, token_tia: AccountId, spaceship: AccountId, boxmall: AccountId, start_block: u64, per_block_reward: U128, slot_fee2: U128, slot_fee3: U128) -> Self {
    ...
    pub fn claim( &mut self ) {
        ...
        let mut msg: String = String::from(r#"{"contract_id": "trialpool","user_id": ""#);
...
impl Contract{
    pub fn internal_buy_slot( &mut self, sender_id: AccountId, amount: u128, slot_index: u64)  {
        ...
        let mut stake_slot_vec: Vec<StakeSlot> = self.slot_info.get(&sender_id).unwrap_or(
            vec![StakeSlot{token_id:"".to_string(),enable: false};4]
        );
    ...
    pub fn internal_stake( &mut self, sender_id: AccountId, slot_index: u64, token_id: TokenId) {
        ...
        let mut stake_slot_vec: Vec<StakeSlot> = self.slot_info.get(&sender_id).expect("No_StakeSlot");
...
impl Contract{
    ...
    pub fn internal_init_slot(&mut self) {
        self.slot.push(&Slot{price : 0, enable : true});
        self.slot.push(&Slot{price : 0, enable : true});
        self.slot.push(&Slot{price : self.slot_fee2, enable : false});
        self.slot.push(&Slot{price : self.slot_fee3, enable : false});
    }
