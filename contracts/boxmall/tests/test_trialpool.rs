...
fn deploy_contracts(
    ...
    ( boxmall_contract, usn_contract, magicbox_contract, spaceship_contract, shippool_contract, 
      token_tia_contract, riskerpool_contract, rankpool_contract, trialpool_contract )
}
...
fn test_trialpool() {
    ...
    let (boxmall, mock_usn,  magicbox, spaceship, shippool, 
        token_tia, riskerpool, rankpool, trialpool ) = deploy_contracts(
        &root, 
        bank.account_id(), 
        bank_u.account_id(), 
        oracle.account_id(),
    ...
    */
    ...
    // 8.1 get nft by spaceship.nft_tokens_for_owner
    // 8.2 stake nft by spaceship.nft_transfer_call
    // 8.3 get_slot_list 
    println!("------------------------------ 8.0 buy slot---------------------------");
    let msg: String = String::from("2");
    out_come = call!(
        user,
        token_tia.ft_transfer_call(trialpool.user_account.account_id(), to_yocto18("300").into(), None, msg),
        deposit = 1
    );
    out_come.assert_success();
    println!("{:#?}", out_come.promise_results());
    ...
    out_come = call!(
        ...
        spaceship.nft_transfer_call(
            trialpool.user_account.account_id(), 
            //"1:4:1:11".to_string(), 
            "1:1:30:11".to_string(), 
            None,
            None,
    ...
    out_come = call!(
        user,
        trialpool.get_slot_list(user.account_id()),
        1,
        near_sdk_sim::DEFAULT_GAS
    );
