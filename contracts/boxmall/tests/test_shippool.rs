...
fn deploy_contracts(
    ...
    (boxmall_contract, usn_contract, magicbox_contract, spaceship_contract, shippool_contract, token_tia_contract, riskerpool_contract, rankpool_contract)
}
...
fn test_shippool() {
    ...
    let user1 = root.create_user(AccountId::new_unchecked("user1".to_string()), to_yocto("1000"));
    let user2 = root.create_user(AccountId::new_unchecked("user2".to_string()), to_yocto("1000"));
    let user3 = root.create_user(AccountId::new_unchecked("user3".to_string()), to_yocto("1000"));
    ...
    let user_balance = user1.account().unwrap().amount;
    println!("{:#?}",user_balance);


    let (boxmall, mock_usn,  magicbox, spaceship, shippool, token_tia, riskerpool, rankpool ) = deploy_contracts(
        &root, 
        bank.account_id(), 
        bank_u.account_id(), 
        oracle.account_id(),
    ...
    //
    // mint usn for user1
    register_and_fund_mock_usn_account(&mock_usn, &mock_usn.user_account, user1.account_id().clone(),2000);
    let balance = view!(mock_usn.ft_balance_of(user1.account_id()));
    println!("user1: mock_usn.ft_balance_of: {:#?}", balance.unwrap_json_value());

    //
    // mint usn for user2
    register_and_fund_mock_usn_account(&mock_usn, &mock_usn.user_account, user2.account_id().clone(),2000);
    let balance = view!(mock_usn.ft_balance_of(user2.account_id()));
    println!("user2: mock_usn.ft_balance_of: {:#?}", balance.unwrap_json_value());
    ...
    //////////////////////////////////////////////
    // register and fund 'user1' account
    register_and_fund_tia_account(&token_tia, &owner, user1.account_id().clone(), 2000);

    let balance = view!(token_tia.ft_balance_of(user1.account_id()));
    println!("user1: token_tia.ft_balance_of: {:#?}", balance.unwrap_json_value());
    //

    //////////////////////////////////////////////
    // register and fund 'user2' account
    register_and_fund_tia_account(&token_tia, &owner, user2.account_id().clone(), 2000);

    let balance = view!(token_tia.ft_balance_of(user2.account_id()));
    println!("user2: token_tia.ft_balance_of: {:#?}", balance.unwrap_json_value());
    //
    ...
    // near call $BOXMALL set_num_limit '{"limit": 8}' --account_id=$OWNER
    println!("------------------------------ 3 ------------------------------");
    out_come = call!(
        owner,
    ...
    out_come = call!(
        ...
        //boxmall.bind(user1.account_id()), // should be error
        //deposit = 1
    ...
    */
    ...
    out_come = call!(
        owner,
        boxmall.ship_wallet_add(user1.account_id(),U128(to_yocto18("30"))),
        //deposit = 1
        gas = near_sdk_sim::DEFAULT_GAS
    );
    out_come.assert_success();
    ...
    out_come = call!(
        owner,
        boxmall.ship_wallet_balance_of(user1.account_id()),
        //deposit = 1
        gas = near_sdk_sim::DEFAULT_GAS
    );
    out_come.assert_success();
    ...
    */
    for _i in 0..10{
        let msg: String = String::from("{\"box_type\": \"buy_s\",\"num\": 5}");

        out_come = call!(
            user1,
            token_tia.ft_transfer_call(boxmall.user_account.account_id(), to_yocto18("100").into(), None, msg),
            deposit = 1
        );
        out_come.assert_success();
        println!("{:#?}", out_come.promise_results());
    ...
    out_come = call!(
        user1,
        magicbox.get_box_list(user1.account_id(), 0, 100),
        deposit = 1
    );
    ...
    // 7.2 set_num_limit
    println!("------------------------------ 7.2.1 magicbox.set_num_limit");

    out_come = call!(
        owner,
        magicbox.set_num_limit(20),
        //deposit = 1
        gas = near_sdk_sim::DEFAULT_GAS
    );
    out_come.assert_success();
    ...
    println!("------------------------------ 7.2.2 shippool.set_pioneer_max");

    out_come = call!(
        owner,
        shippool.set_epoch_pioneer_count(1),
        //deposit = 1
        gas = near_sdk_sim::DEFAULT_GAS
    );
    out_come.assert_success();
    ...
    // 7.3 openbox
    println!("------------------------------ 7.3.1.1 magicbox.open_box");
    //let box_token_ids: Vec<String> = vec!["0:1".to_string(),"1:1".to_string(),"2:2".to_string(),"3:2".to_string()];
    let box_token_ids: Vec<String> = vec![
        "0:2".to_string(),"1:2".to_string(),"2:2".to_string(),"3:2".to_string(),"4:2".to_string(),
        "5:2".to_string(),"6:2".to_string(),"7:2".to_string(),"8:2".to_string(),"9:2".to_string()
        ];
    out_come = call!(
        user1,
    ...
    println!("------------------------------ 7.3.1.2 magicbox.open_box");
    let box_token_ids: Vec<String> = vec![
        "10:2".to_string(),"11:2".to_string(),"12:2".to_string(),"13:2".to_string(),"14:2".to_string(),
        "15:2".to_string(),"16:2".to_string(),"17:2".to_string(),"18:2".to_string(),"19:2".to_string()
        ];
    out_come = call!(
        user1,
        magicbox.open_box(box_token_ids),
    ...
    println!("------------------------------ 7.3.1.3 magicbox.open_box");
    let box_token_ids: Vec<String> = vec![
        "20:2".to_string(),"21:2".to_string(),"22:2".to_string(),"23:2".to_string(),"24:2".to_string(),
        "25:2".to_string(),"26:2".to_string(),"27:2".to_string(),"28:2".to_string(),"29:2".to_string()
        ];
    out_come = call!(
        user1,
        magicbox.open_box(box_token_ids),
    ...
    println!("------------------------------ 7.3.2 shippool.get_current_epoch");
    out_come = call!(
        user1,
        shippool.get_current_epoch(),
        deposit = 1
    );
    out_come.assert_success();
    println!("{:#?}", out_come.promise_results());
    ...
    println!("------------------------------ 7.3.3 shippool.get_epoch_history_list");
    out_come = call!(
        user1,
        shippool.get_epoch_history_list(1,100),
        //deposit = 1
        gas = near_sdk_sim::DEFAULT_GAS
    );
    out_come.assert_success();
    ...
    /*
    println!("------------------------------ 7.3.4 shippool.get_user_info");
    out_come = call!(
        user1,
        shippool.get_user_info(user1.account_id()),
        //deposit = 1
        gas = near_sdk_sim::DEFAULT_GAS
    );
    out_come.assert_success();
    ...
    println!("------------------------------ 7.3.5 shippool.pioneer_claim");
    out_come = call!(
        user1,
        shippool.pioneer_claim(0),
        //deposit = 1
        gas = near_sdk_sim::DEFAULT_GAS
    );
    out_come.assert_success();
    ...
    println!("------------------------------ 7.3.6 shippool.pioneer_claim");
    out_come = call!(
        user1,
        shippool.pioneer_claim(1),
        //deposit = 1
        gas = near_sdk_sim::DEFAULT_GAS
    );
    out_come.assert_success();
    ...
    println!("{:#?}", get_logs(&out_come)); 
    */
    println!("------------------------------ 7.3.7 shippool.get_epoch_history_list");
    out_come = call!(
        user1,
        shippool.get_epoch_history_list(1,100),
        //deposit = 1
    ...
    //
    // 7.4 get shipwallet balance to check balance
    //
    println!("------------------------------ 7.4 ------------------------------");
    out_come = call!(
        owner,
