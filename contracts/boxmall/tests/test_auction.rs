...
use auction::ContractContract as auction;
...
fn deploy_contracts(
    ...
    let auction_contract = deploy!(
        contract: auction,
        contract_id: "auction".to_string(),
        bytes: &AUCTION_WASM_BYTES,
        signer_account: root,
        init_method: new(owner_id.clone(),"spaceship".parse().unwrap(),"token_tia".parse().unwrap(),team_id.clone())
    );
    println!("deploy auction ... done");
    //
    ...
    (boxmall_contract, usn_contract, magicbox_contract, spaceship_contract, shippool_contract, 
        token_tia_contract, riskerpool_contract, rankpool_contract, auction_contract)
}
...
fn test_auction() {
    ...
    let team = root.create_user("team".parse().unwrap(), to_yocto("100"));

    let user_balance = user.account().unwrap().amount;
    println!("{:#?}",user_balance);


    let (boxmall, mock_usn,  magicbox, spaceship, shippool, token_tia, riskerpool, rankpool, auction ) = deploy_contracts(
        &root, 
        bank.account_id(), 
        bank_u.account_id(), 
        oracle.account_id(),
    ...
    //////////////////////////////////////////////
    // register and fund 'auction' account
    register_and_fund_tia_account(&token_tia, &owner, auction.account_id().clone(), 2000);

    let balance = view!(token_tia.ft_balance_of(auction.account_id()));
    println!("auction: token_tia.ft_balance_of: {:#?}", balance.unwrap_json_value());
    //

    //////////////////////////////////////////////
    // register and fund 'auction' account
    register_and_fund_tia_account(&token_tia, &owner, team.account_id().clone(), 2000);

    let balance = view!(token_tia.ft_balance_of(team.account_id()));
    println!("team: token_tia.ft_balance_of: {:#?}", balance.unwrap_json_value());
    //
    ...
    */
    ...
    let box_token_ids: Vec<String> = vec!["0:2".to_string(),"1:2".to_string(),"2:2".to_string(),"3:2".to_string(),"4:2".to_string()];
    out_come = call!(
        user,
    ...
    let box_token_ids: Vec<String> = vec!["5:2".to_string(),"6:2".to_string(), "7:2".to_string(),"8:2".to_string(),"9:2".to_string()];
    out_come = call!(
        user,
        magicbox.open_box(box_token_ids),
        deposit = 1
    ...
    //
    // 8 get nft from spaceship contract for owner 
    //
    println!("------------------------------ 8.1 get nft from spaceship contract for owner");
    out_come = call!(
        owner,
        spaceship.nft_tokens_for_owner(user.account_id(),Some(U128(0)), Some(1000)),
        deposit = 1
    );
    out_come.assert_success();
    println!("{:#?}", out_come.promise_results());
    ...
    println!("------------------------------ 8.2 check nft supply for user");
    out_come = call!(
        owner,
        spaceship.nft_supply_for_owner(user.account_id()),
        deposit = 1
    );
    out_come.assert_success();
    println!("{:#?}", out_come.promise_results());
    ...
    //
    println!("------------------------------ 8.3 add nft to auction");
    //let msg: String = String::from("{\"price\": \"100\",\"up_price\": \"2\",\"start\": 1655873007,\"end\": 1655973007}");
    out_come = call!(
        owner,
        //auction.add_auction_info("0:1:23:12".to_string(),U128(100),U128(2),1655873007,1655973007),
        // env::block_timestamp = 211000000000
        auction.add_auction_info("0:1:23:12".to_string(),U128(to_yocto18("100")),U128(to_yocto18("2")),217900020000,257000020000),
    ...
    println!("------------------------------ 8.4 check pool length");
    out_come = call!(
        owner,
        auction.get_auction_count(),
        deposit = 1
    );
    out_come.assert_success();
    println!("{:#?}", out_come.promise_results());
    ...
    // first buy
    println!("------------------------------ 8.5 first buy on auction");
    let msg: String = String::from("0");

    out_come = call!(
        user,
        token_tia.ft_transfer_call(auction.user_account.account_id(), to_yocto18("100").into(), None, msg),
        deposit = 1
    );
    out_come.assert_success();
    println!("{:#?}", out_come.promise_results());
    ...
    // second buy 
    println!("------------------------------ 8.6 second buy on auction");
    let msg: String = String::from("0");

    out_come = call!(
        user,
        token_tia.ft_transfer_call(auction.user_account.account_id(), to_yocto18("102").into(), None, msg),
        deposit = 1
    );
    out_come.assert_success();
    println!("{:#?}", out_come.promise_results());
    ...
    // third buy 
    println!("------------------------------ 8.7 third buy on auction");
    let msg: String = String::from("0");

    out_come = call!(
        user,
        token_tia.ft_transfer_call(auction.user_account.account_id(), to_yocto18("104").into(), None, msg),
        deposit = 1
    );
    out_come.assert_success();
    println!("{:#?}", out_come.promise_results());
