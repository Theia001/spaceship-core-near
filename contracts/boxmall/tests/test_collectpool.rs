...
use collectpool::ContractContract as collectpool;
...
near_sdk_sim::lazy_static_include::lazy_static_include_bytes! {
    ...
    COLLECTPOOL_WASM_BYTES => "../../res/collectpool.wasm",
}
...
fn deploy_contracts(
    ...
    //
    let collectpool_contract = deploy!(
        contract: collectpool,
        contract_id: "collectpool".to_string(),
        bytes: &COLLECTPOOL_WASM_BYTES,
        signer_account: root,
        init_method: new(
            owner_id.clone(),
            "token_tia".parse().unwrap(),
    ...
    println!("deploy collectpool ... done");
    ...
    ( boxmall_contract, usn_contract, magicbox_contract, spaceship_contract, shippool_contract, 
      token_tia_contract, riskerpool_contract, rankpool_contract, collectpool_contract )
}
...
fn test_collectpool() {
    ...
    let (boxmall, mock_usn,  magicbox, spaceship, shippool, 
        token_tia, riskerpool, rankpool, collectpool ) = deploy_contracts(
        &root, 
        bank.account_id(), 
        bank_u.account_id(), 
        oracle.account_id(),
    ...
    //////////////////////////////////////////////
    // register and fund 'collectpool' account with 2975908
    register_and_fund_tia_account(&token_tia, &owner, collectpool.account_id().clone(), 2975908);

    let balance = view!(token_tia.ft_balance_of(collectpool.account_id()));
    println!("collectpool: token_tia.ft_balance_of: {:#?}", balance.unwrap_json_value());
    //
    ...
    */
    ...
    //
    // 8 testing collectpool 
    // 8.0 init_slot
    // 8.0 get nft by spaceship.nft_tokens_for_owner
    // 8.1 stake nft by spaceship.nft_transfer_call
    // 8.2 get_slot_list 
    println!("------------------------------ 8.0 init_slot");
    //   
    out_come = call!(
        owner,
        collectpool.init_slot(),
        1,
    ...
    println!("------------------------------ 8.2 stake nft");
    //
    let msg: String = String::from("1");
    out_come = call!(
        user,
        spaceship.nft_transfer_call(
            collectpool.user_account.account_id(), 
            //"1:4:1:11".to_string(), 
            //"8:1:2:46".to_string(), 
            "1:1:30:11".to_string(), 
            None,
    ...
    println!("transfer_call cost {} Tgas", get_total_gas(&out_come) / 10_u64.pow(12));
    /*
    println!("------------------------------ 8.3 get_slot_list");
    //   
    out_come = call!(
        user,
        collectpool.get_slot_list(user.account_id()),
        1,
        near_sdk_sim::DEFAULT_GAS
    );
    ...
    println!("------------------------------ 8.4 withdraw");
    //   
    out_come = call!(
        user,
        collectpool.withdraw(1),
        1,
        near_sdk_sim::DEFAULT_GAS
    );
