...
type SortitionSumTrees = sortition_sum_tree::SortitionSumTrees;
...
pub const MAX_TREE_LEAVES: usize = 5;
pub const TREE_KEY: &str = "LuckPool/Ticket";
...
pub struct MyNonFungibleToken {
    ...
    sortition_sum_trees: SortitionSumTrees,
}
...
impl MyNonFungibleToken {
            ...
            T: IntoStorageKey,
    {
        ...
        let mut temp = SortitionSumTrees::new();
        temp.create_tree(TREE_KEY.to_string(), MAX_TREE_LEAVES);
    ...
    pub fn mint(
        ...
        self.internal_mint(token_id, token_owner_id, token_metadata, amount)
    }
    ...
    pub fn internal_mint(
        ...
        // self.internal_before_token_transfer(AccountId::new_unchecked("00".to_string()), recipient.clone(), amount, balance);
        let balance = self.balances.get(&token_owner_id).unwrap_or(&0);
        self.internal_before_token_transfer(
            AccountId::new_unchecked("00".to_string()),
            token_owner_id.clone(),
            amount,
            *balance);

        self.token_count += 1;
        let token_id = format!("{}:{}", token_id.clone(), self.token_count);
...
// 原 ticket 方法
impl MyNonFungibleToken {
    pub fn chance_of(&self, user: AccountId) -> u128 {
        self.sortition_sum_trees.stake_of(TREE_KEY.to_string(), user)
    }

    pub fn draw(&mut self, random_number: u128) -> AccountId {
        let bound = self.nft_total_supply();

        if 0 == bound.0 {
            AccountId::new_unchecked("00".to_string())
        } else {
            let value = self.random(random_number, bound.0) as u128;
            self.sortition_sum_trees.draw(TREE_KEY.to_string(), value)
        }
    }

    pub fn create_new_tree(&mut self) {
        let mut temp = SortitionSumTrees::new();
        temp.create_tree(TREE_KEY.to_string(), MAX_TREE_LEAVES);
        self.sortition_sum_trees = temp; // 重新创建一个新树，之前的被新变量覆盖掉了
        self.token_count = 0; // 创建新树的时候这个需要归零
    }

    // 内部方法，看openzeppelin，这里是回调的，应该有个与之配对的afterTokenTransfer，不过原合约没有实现这个方法
    pub fn internal_before_token_transfer(&mut self, from: AccountId, to: AccountId, amount: u128, balance: u128) {
        if from == to {
            return;
        }

        if from != AccountId::new_unchecked("00".to_string()) {
            // 看openzeppelin这里from是空
            // let from_balance = self.ft_balance_of(from.clone()).0 - amount;
            let from_balance = 0 - amount;
            self.sortition_sum_trees.set(TREE_KEY.to_string(), from_balance, from);
        }

        if to != AccountId::new_unchecked("00".to_string()) {
            // let to_balance = self.ft_balance_of(to.clone()).0 + amount;
            let to_balance = balance + amount;
            self.sortition_sum_trees.set(TREE_KEY.to_string(), to_balance, to.clone());
            self.balances.insert(to.clone(), to_balance);
        }
    }

    pub fn random(&self, lower_bound: u128, mut upper_bound: u128) -> u64 {
        ...
        for i in lower_bound..upper_bound {
            ...
            for j in 0..8 {
                temp_seed = temp_seed * 10 + seeds[(i * 4 + j) as usize] as u128;
            }
            seed += temp_seed;
        }
